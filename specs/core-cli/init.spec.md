...
13. **Desktop scheduled tasks (design §13 step 12).** Write one `~/.claude/scheduled-tasks/<task-name>/SKILL.md` per scheduled loop — the twelve named in the design (hygiene, distil, skill-suggest, anatomy-refresh-deep, rule-decay, atlas-staleness, onboarding-drift, spec-drift, specflow-lint, specflow-verify, test-runner, bug-triage) — each `name`/`description` frontmatter plus the prompt body per schema §9-adjacent conventions. Cadence is confirmed by the user in the Desktop app on first open (design open question #11, conservative path). Idempotent: existing task files are overwritten only with `--force`, otherwise left in place.
