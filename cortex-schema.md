...
**Recall pointer lines (3.4 second revision; third shape at the third revision) — the grammar, recorded once.** Three line shapes: the first two emitted by the `PreToolUse` (Grep/Bash) hook and, the second shape only, by the PreRead recall marker; the third by the `UserPromptSubmit` hook. `pulse.usage` Rule 11 counts fired and followed pointers by the line prefixes `Recall:`, `Decided:`, `Evidence:` and `Open:` (the PreRead marker begins with `Evidence:` or `Open:` when its `Decided:` part is empty):
```
...
| `check.config` | `cortex.config.json` + version; `profile` enum (3.3); (3.4 third revision) `hooks.readDefer` boolean when present → non-boolean is an error, `true` with `hooks.preRead: false` a warning | §10 | error (warning for the readDefer/preRead pairing and unknown keys) |
