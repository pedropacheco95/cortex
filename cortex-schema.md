...
**The trust contract (carried forward, unchanged in substance).** Insight is inferred, not curated: **context, not authority.** Where insight conflicts with a compass rule or a spec, the gated layer wins. It never carries write-time enforcement authority (no PreWrite reads insight, §5) and, with two narrow exceptions, no hook injects it: the SessionStart observations digest (§4.10.11, new at 3.1) — a small, separately-budgeted, qualifying-entries-only surfacing of `insight/observations/` — and, new at 3.3, insight **concept names** inside the SessionStart coverage map (§5), names only and never bodies. Neither is a general insight-injection precedent; §5 records the measurement that motivated the 3.3 widening and the reasoning that keeps it narrow. Committed in full (Decision 1's fourth git-policy quadrant: machine-owned **and** committed).
...
```json
{
  ...
  "subjects": {
    ...
    "R-001": { "decided": ["decision.2026-07-07-five-module-architecture"], "evidence": [], "threads": [], "observations": ["working-style"] },
...
**Entries.** One row per scanned artefact whether or not it contributed a subject — `kind` (`decision | evidence | thread | observation`), `title` (the frontmatter `title`; a thread's key text cut to 80 characters; an observation's theme), `path` (project-relative POSIX), `date` (`date`, `opened`, or `updated` respectively), `keywords` (the title's lowercase tokens of three or more characters, split on non-alphanumerics, plus every `bears_on` ref verbatim — sorted, deduplicated; **never body text**, so the index carries no knowledge content, only names, which is the same line §5's coverage map holds).
...
|---|---|---|---|
| `SessionStart` | new session | Pointer block (below, incl. the 3.3 **entry line** under the `specflow` profile), plus a qualifying-entries **observations digest** (§4.10.11) when `insight/observations/` has one, plus the 3.4 **coverage map** and **rationalization table** when `.cortex/` holds anything worth listing | <100 tok pointer block; the digest separately budgeted at ≤150 tok; the coverage map separately budgeted at **≤1,800 tok**; the table separately budgeted at **≤250 tok**. Four pools, never folded into one another — each section is omitted entirely rather than trimmed at another's expense |
...
{{#if coverage map non-empty}}
## What Cortex holds here
{{COVERAGE_MAP}}
{{/if}}
{{#if coverage map non-empty}}
## Before you grep for it
...
**Coverage map and rationalization table (new at 3.3).** `{{COVERAGE_MAP}}` is a generated table of contents for the project's knowledge layer — compass rules with their `governs`, atlas decisions, domain terms, stakeholders, observation themes, archive document count, and insight **concept names** — rendered fresh from disk on every fire, with no stored artefact and no refresh command. `{{RATIONALIZATION_TABLE}}` is static text naming the reasons a session skips consulting Cortex and answering each. Both are separately budgeted (≤1,800 and ≤250 tok), both are omitted entirely when the map would be empty, and neither ever carries knowledge **content** — the map lists names, identifiers, scopes, and paths, never the body of a rule, the reasoning of a decision, or the definition of a term. Over budget, the map collapses whole categories to a count-and-pointer line in a fixed order (observations → stakeholders → domain terms → decisions → concepts); **compass rules never collapse**, because a rule the assistant does not know about is the one failure this map must not produce. Owned by `scaffolding.coverage-map` and `scaffolding.rationalization-table`; carried by `hooks.session-start` Rules 10 and 11.
...
**Recall pointer lines (3.4 second revision) — the grammar, recorded once.** Two line shapes, emitted by the `PreToolUse` (Grep/Bash) hook and, the second shape only, by the PreRead recall marker; `pulse.usage` Rule 11 counts fired and followed pointers by these two prefixes:
```
Recall: {{KIND}} {{YYYY-MM-DD}} {{TITLE cut to 60}} ({{PATH}})
Decided: {{DECISION_ID}} · Open: {{T-ID}} {{THREAD KEY TEXT cut to 40}}
```
`{{KIND}}` is `decision | evidence | thread | observation`; `{{PATH}}` is the entry's project-relative path from the index; the PreRead marker's `Decided:` form carries lists — `Decided: <ids, max 3> · Evidence: <ids, max 2> · Open: <T-ids, max 2>` — with empty parts omitted. The last line MAY end with ` · more: cortex why {{REF}}` when the subject holds more than was shown. Every token is a name, an id, a date or a path taken from `recall-index.json`; **no line ever carries body text** (the index has none, §4.11) and **no line is an imperative** — no "read", "consult", "check", no second person: Claude Code injects its own autonomy text, and a Cortex line that instructs is a line that gets learned as ignorable (the 2026-09-02 Fable 5.1 adaptation audit). Titles are cut on a word boundary with a trailing `…`; ids are never cut. Owned by `hooks.search-annotate` Rule 7.

**Envelope (pinned to the Claude Code hooks API, verified 2026-07-02).** All Cortex hooks communicate via **exit 0 + stdout JSON**: SessionStart emits `{"hookSpecificOutput": {"hookEventName": "SessionStart", "additionalContext": …}}`; the PreWrite warning, the PreRead summary and — 3.4 second revision — the Grep/Bash pointer emit `{"hookSpecificOutput": {"hookEventName": "PreToolUse", "permissionDecision": "allow", "additionalContext": …}}` (the pointer hook emits it on a match only, empty stdout otherwise); PostWrite, PostRead, SessionEnd and Stop emit nothing (empty stdout). No Cortex hook ever exits 2, exits non-zero, or emits `deny`/`ask`, and the Stop hook never emits a `decision` — warn-never-block is enforced by the envelope itself. Hook-internal errors degrade (operation proceeds) and append to `pulse/reports/hook-errors.md` (§4.5). Registration entries use the command signature `cortex hook <name>` — that prefix is the **ownership marker** (the JSON transposition of §8's CLAUDE.md marker idiom); tooling manages only entries carrying it.

**No insight hook, save two narrow exceptions (v2.0 stance, carved out at 3.1, widened once in the 3.3 revision).** The insight module adds no hook of its own and, PreRead/PostRead aside, no field to any *other* existing hook payload. Insight is pull-first via the CLI (§4.10.8) for everything except two things: the SessionStart row carries the qualifying-entries **observations digest** (§4.10.11, new at 3.1), and — new at 3.3 — insight **concept names** inside the coverage map. Injecting unreviewed inferred *assertions* at SessionStart or PreRead beyond these would still spend the trust budget on the layer with the weakest trust warrant, and the enforcement channel (PreWrite) reads compass precisely because compass is gated (v2 design §7.4; addendum A1.5, A4.0).

**Why the 3.3 widening, and why it is narrow.** The pull-only stance rested on a premise — that pull works — which measurement falsified. `pulse.usage` over 55 sessions of the Cortex repo itself recorded **2** `cortex insight` invocations in total, **0** for `concept`, **0** for `element`, against **76** searches into `.cortex/` — and one session invoked `cortex insight scopes`, a verb that has never existed, which is what "I do not know what is available" looks like from the outside. A surface nobody can find is not a trust-preserving surface; it is an unused one.

The widening is deliberately the smallest thing that fixes it: **concept names only — never concept bodies, never per-file entries, never any other inferred content.** A concept *name* is a label, not a claim: `hook-safety` asserts nothing about the code that could be wrong in the way an inferred purpose or connection could. So the trust argument above survives intact for everything it was actually protecting, and the injected surface stays a table of contents rather than an answer. Recorded as an atlas decision (2026-08-05). The hook table above is otherwise unchanged in this regard from v1/v2.0.

**Validated by** `check.hook-config`: the hook entries `cortex init` (or `cortex sync`, refreshing an existing project's registration) writes to `.claude/settings.json` match the registered hooks; the Read-pair entries (PreRead + PostRead, together) are present iff `cortex.config.json` `hooks.preRead` is true — which is the default; and (3.3 third revision) the `SessionEnd` entry `cortex hook session-end` and the `Stop` entry `cortex hook stop` are both present whenever the settings file carries **any** `cortex hook ` entry (a project whose hooks Cortex manages must carry the whole set — the remedy the violation names is `cortex sync`; a settings file with no Cortex-owned entry at all is not implicated, as before); and (3.4 second revision) the `PreToolUse` entry `cortex hook search-annotate` with matcher `Grep|Bash` is required under the same whole-set condition — it is not behind `hooks.preRead`, which is the Read pair's flag alone. The payload *text* is the hooks' contract with Claude, asserted by the hook specs' tests, not by the validator.
...
| `check.hook-config` | registered hooks match config; the Read pair iff `hooks.preRead`; (3.3 third revision) `SessionEnd` and `Stop` present whenever any Cortex-owned entry is; (3.4 second revision) the `PreToolUse` `Grep\|Bash` entry `cortex hook search-annotate` present under the same whole-set condition | §5 | error |
...
| `check.recall-index` (new at 3.4) | `recall-index.json` shape (only when the file exists): required keys, `schemaVersion` string, subject lists are string arrays whose ids exist in `entries`, entry `kind` in enum, `path` non-empty | §4.11 | error |
