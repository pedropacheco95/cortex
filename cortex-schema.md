...
| `PreToolUse` (Read) | before a read | **Schema-3.0 gap, flagged rather than fabricated:** the v2.0 payload read a target path's row in `anatomy/files.md`; that data source is removed (addendum A7.3). Unlike PreWrite's compass re-root (§5, addendum A1.5 — a same-shape path swap, rule files being identical before and after), re-pointing this hook to `insight` is not mechanical: insight's per-file entry (§4.10.2) is a multi-section markdown document, not a table row, so this contract would still need to decide (a) which entry section(s) substitute for the legacy `{{PURPOSE}}`/`{{SPEC_LINKS}}`/`{{APPLICABLE_RULE_IDS}}` fields inside the <75-token budget, and (b) what the hook injects when the target path has no insight entry yet. Design §5.10 names the *content* mapping (anatomy's purpose lines become insight's L2 output; `spec_links`/`governs` resolution moves to insight's Connections section) but not the *payload* shape — both remaining decisions are owned by build-order-v3 step 7, not this fold-in. Until step 7 lands, the hook has no data source and SHOULD emit nothing (silently degrading, per the warn-never-block/never-fabricate discipline). | <75 tok (v2.0 template; void pending step 7) |
