...
|---|---|---|---|
| `SessionStart` | new session | Pointer block (below, incl. the 3.3 **entry line** under the `specflow` profile), plus a qualifying-entries **observations digest** (§4.10.11) when `insight/observations/` has one | <100 tok pointer block; the digest is separately budgeted at ≤150 tok on top (never folded into the 100), omitted entirely when no entry qualifies |
