...
**3.3 is a MINOR bump** (§10.2 — additive, backward-compatible: a new optional config field): one addition — `cortex.config.json` `profile` (§10.1), the process-profile selector, plus its `check.config` enum validation. The key is **optional with a default**, so a config predating it validates clean and behaves exactly as before. Nothing existing is removed, renamed, or made required, and no migration ships.
