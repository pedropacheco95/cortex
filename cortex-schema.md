...
| `promotion` | (v2.0: `insight-gaps`, retired at v3.0; no active v3.0 producer — retained per addendum A7.4, not repurposed) | `.cortex/compass/`, `.cortex/atlas/`, `RULES.md` | append **or** create | apply the payload to the gated target, inject a `source:` back to the originating artefact; **the v2.0 "mark the insight original promoted" trailer (§4.10.4, v2.0) has no v3.0 equivalent** — a v3.0-derived promotion would cite its origin via `provenance:` (§6, A6) instead |
| `gated-layer-update` | (v2.0: `insight-gaps` signal 4-gated, retired; no active v3.0 producer — retained per addendum A7.4) | `.cortex/compass/`, `.cortex/atlas/`, `RULES.md` | **edit** | replace the named byte-range in the target (§4.5.2); a correction to existing gated content is an edit, not an append |
...
Notes: `promotion` and `gated-layer-update` are retained per addendum A7.4 (explicitly not repurposed) but have **no active v3.0 producer** — insight enrichment is now a direct, ungated write by the insight-refresh loops and `cortex-loop-session-observe` (§4.10, §9; addendum A7.3), not a proposal. `user-directed-capture` remains the only type that MAY target `.cortex/insight/`, because an explicit "remember this" deserves explicit confirmation of *where* it landed even when the landing is ungated. `decision-candidate`'s permitted root is `atlas/decisions/` specifically (not all of `atlas/`), since it targets exactly the sole-home decisions directory (§4.3).
...
**Insight preset (deferred at v3.0).** The v2.0 insight preset (a constellation overlay over `insight/map/graph.json`'s node/edge set) is **dropped** — that node set no longer exists (`insight/` is rebuilt wholesale, §4.10, addendum A4.6 note). A v3.0 constellation preset over the new code-understanding graph (`insight/graph.json`/`clusters.json`, §4.10.6) is an **open question** (design §11 Q2) and, if adopted, is its own later build-order-v3 step (step 10) — not specified by this contract. `constellation.json` stays **curated-only** regardless of that decision: its shape and byte-determinism contract are unchanged, and it never contains inferred edges or clusters.
...
| `PostToolUse` (Write/Edit) | after a write | **Nothing (v3.0).** The v2.0 anatomy writeback (`tokens`/`sha256`/`last_seen`/`needs_purpose_refresh` on an `anatomy/files.md` row) is removed — `anatomy/` no longer exists (addendum A7.3). No replacement side-effect is specified by schema 3.0; read-time purpose capture into insight is **design-deferred to build-order-v3 step 7** (anatomy deprecation, design §5.10). | n/a |
| `PreToolUse` (Read) | before a read | **Schema-3.0 gap, flagged rather than fabricated:** the v2.0 payload read a target path's row in `anatomy/files.md`; that data source is removed (addendum A7.3). This contract does not yet specify a v3.0 replacement — build-order-v3 step 7 re-points this hook to `insight` (design §5.10). Until step 7 lands, the hook has no data source and SHOULD emit nothing (silently degrading, per the warn-never-block/never-fabricate discipline). | <75 tok (v2.0 template; void pending step 7) |
| `PostToolUse` (Read) | after a read | **Nothing (v3.0).** The v2.0 capture half (`<cortex:purpose>` tag sweep → `purpose_source: read-time` on an anatomy row) is removed with anatomy (addendum A7.3); its successor (read-time-tagged insight extraction metadata) is **design-deferred to build-order-v3 step 7**. | n/a |
...
**PreToolUse (Read) — v3.0 status.** The v2.0 template below is **void** at schema 3.0 (its data source, `anatomy/files.md`, no longer exists) and is preserved here only so the gap is visible, not silently dropped:
```
[v2.0, void] {{PATH}}: {{PURPOSE}} (~{{TOKENS}} tok). Specs: {{SPEC_LINKS}}. Rules: {{APPLICABLE_RULE_IDS}}.
[v2.0, void] If this purpose is wrong or stale after reading, emit: <cortex:purpose file="{{PATH}}">corrected one-line purpose</cortex:purpose>
```
Build-order-v3 step 7 (anatomy deprecation) re-points this hook's data source to `insight` per-file entries (design §5.10) and re-specifies the payload; that re-specification is out of scope for this fold-in.

**PostToolUse (Read) — v3.0 status.** The v2.0 capture half (sweeping the transcript for `<cortex:purpose>` tags and writing `purpose_source: read-time` to an anatomy row) is **removed** (addendum A7.3). No replacement mechanic is specified here; build-order-v3 step 7 integrates read-time capture into insight extraction metadata (design §5.10).
...
The `<!-- cortex:start … -->`/`<!-- cortex:end -->` markers delimit the managed block so `cortex init` can update it idempotently without touching the rest of CLAUDE.md. *(v3.0 interpretive note: the addendum (A1.8, A4.8) specifies the `## Cortex` bullet re-root and the `## Cortex Insight` block verbatim; the one-line `archive/` bullet above is this schema's own necessary addition for a module the addendum introduces but didn't separately spec a CLAUDE.md bullet for — flagged for review.)* **Validated by** `check.claude-md`: if CLAUDE.md exists, the managed block is well-formed and its `v…` matches `cortex.config.json`.
