...
- **Freshness is stated, not assumed (2026-09-17).** Every per-file answer now says when it was built and whether the file has changed since (`insight.cli` Rule 9; the same mark rides on the read-time summary the assistant sees before opening a file). Business rule 5 is unchanged — this is rule 3's honest miss applied to time: an answer that may be out of date says so instead of passing as current.
