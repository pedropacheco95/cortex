...
- `specflow.entry-gate` — `specflow-entry` (renamed from `specflow-change-router`): the process gate above the profile. Keeps the whole classification contract, adds the Iron Law, the priority rule (process skills before implementation skills), and a rationalization table; re-armed each session by the SessionStart entry line.
...
- `specflow.intent-reconcile` — `specflow-intent-reconcile`: pin a verbatim ask with a RED anchor test, then decide **counterfactually** whether the spec-derived test would fail without the specific behaviour. Subsumed → retire the anchor into the register; not subsumed → file a missing-criterion bug and flag it. The artefact it writes is `archive.intent-register`.
