...
6. **The locked preset set.** Four presets (build-order-v3 step 10 added the fourth, `insight`, previously deferred — design §11 Q2); unknown preset → `400`; `domain` required for `domain` preset else `400`:
   ...
   | `insight` | *not* a curated node filter — a wholly separate, self-contained composition over `.cortex/insight/graph.json`+`clusters.json`, built fresh per request outside `applyPreset`'s curated filtering and never touching `constellation.json` (see `constellation.insight-preset-v3`, this preset's own spec) |

7. **Filter closure.** (Curated presets only — `default`/`orphans`/`domain`; `insight`'s own composition rules live in `constellation.insight-preset-v3`.) For every preset: edges are included iff **both endpoints** are in the filtered node set; groups/children are pruned to those with at least one remaining node; `counters` are recomputed over the filtered sets. A `domain` value matching nothing returns `200` with empty `nodes`/`edges` (an honest empty lens, not an error).
8. **The SPA.** `GET /` serves one self-contained HTML page: a full-viewport 2D `<canvas>` that renders the map's three structural tiers — top-level groups (domains, one signature hue per branch), child groups (the glowing "stars"), and artefact nodes (the innermost dots). The dissolve mechanic is the core interaction: a child group shows as a star when zoomed out and **dissolves** into its member artefacts past a zoom threshold, re-condensing on zoom out. The zoom-driven level-of-detail/dissolve threshold math lives in `src/constellation/lod.ts` (pure, exported, unit-tested there) and is embedded verbatim into the served inline script so shipped and tested logic cannot drift. Around the canvas, glass-panel chrome: a top bar (logo, mono stats line, search, and the preset switcher naming exactly the four presets), a breadcrumb, a legend, zoom controls, a one-time hint, a hover tooltip, and a slide-in detail panel. Positions are deterministic (only the camera transform animates). The single external resource is the Google Fonts stylesheet (with fallback stacks so the page reads offline); there is no external `<script src>`. Fine-grained visual behaviour beyond these landmarks is deliberately unpinned at this layer (journey tier, deferred).
...
- **And** `GET /` returns HTML containing the `<canvas id="constellation">` element, the glass-chrome landmarks (stable hooks `id="stats"`, `id="search"`, `id="presets"`, `id="breadcrumb"`, `id="legend"`, `id="zoom-controls"`, `id="hint"`), and a preset switcher naming exactly `default`, `orphans`, `domain`, `insight` (via `data-preset="<name>"`, the hook the tests key off)
...
- **Given** the v2 anatomy lenses (`anatomy-only`, `knowledge-only`), retired in v3.0 (the v2 serve-time `insight` overlay this once listed alongside them is no longer retired — build-order-v3 step 10 shipped a real, differently-designed v3 `insight` preset; see `constellation.insight-preset-v3`)
- **When** `GET /api/constellation?preset=<retired>` for either of them
- **Then** the response is `400` with a body naming the four valid presets (`default, orphans, domain, insight`)
...
- **Then** the response is `400` with a body naming the four valid presets
...
- **The fourth preset, `insight` (build-order-v3 step 10), is specified separately** in `constellation.insight-preset-v3` — its own composition rules, rendering discipline, and acceptance criteria live there rather than being folded into this spec's Rules 6-9, since its composition is architecturally distinct (not a curated `applyPreset` filter). This spec's own Rules only had to change to (a) recognize `insight` as a fourth, real, switcher-facing name, and (b) stop describing it as retired.
