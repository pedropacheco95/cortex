...
**Final mechanism (2026-07-09, owner-approved): option 2 via skill.** After option 1 shipped, reverse-engineering the app bundle proved the registry is **in-memory per launch** — loaded once from `scheduled-tasks.json` at app start and rewritten wholesale from memory on every task event, so an external write while the app runs is silently clobbered, and a single malformed field makes the app treat the whole file as empty and wipe every task on its next flush. Direct writing was therefore **demoted to a guarded, app-closed-only fallback**, and the shipped path is registration through the app's own sanctioned surface:

- **`cortex init` sets everything up headlessly and prints instructions** — payloads written, then a read-only `registrationStatus` check (registry-not-found tolerated) drives the summary: an instruction block ("open this folder in Claude Desktop and say `run cortex-register-tasks`, then confirm with `cortex tasks verify`") when any of the fourteen is unregistered, or the all-registered one-liner.
- **`cortex tasks plan [--json]`** — new verb: Core's authoritative registration plan (scoped id, cron, cwd, permissionMode, payload path, description per task; stable `planVersion: 1` JSON) so the skill never hardcodes cadences.
- **The `cortex-register-tasks` skill** (both trees, byte-identical) — runs in a Claude Desktop session, where the app exposes its internal `mcp__scheduled-tasks__*` MCP tools (`create_scheduled_task` requires the SKILL.md to already exist at the id-derived path; id must equal the payload dir name). It preflights tool availability, diffs the plan against `list_scheduled_tasks`, creates missing / updates drifted tasks only for this project, and finishes with `cortex tasks verify`.
- **`cortex tasks register` guard** — refuses (exit 1, before any write, no escape hatch) while the Desktop app runs, via an injected process check (`desktopAppRunning`: `pgrep -f` against the app bundle's main binary, macOS-only); the refusal message explains the clobber/wipe hazard and points at the skill flow. `verify` stays unrestricted (read-only).

Contract updates in the same change: `core-cli.tasks-register` (plan verb, guard, skill flow primary / direct write demoted), `core-cli.init` Rules 13/15 + AC + Notes OQ#11, `cortex-schema.md` §9.1 mechanism paragraph.

**The original option-1 shipment (retained as the fallback's record):**
