...
source_sha256: "718e9569f1f4a294be653de6d037167a07593fdffc5e0747c81c432934d303d9"
---
# src/cli/templates.ts

## Purpose
The single source of every literal string `cortex init` (and, for the scaffolding mechanism it shares, `cortex sync`) writes to disk — schema version and config defaults, gitignore lines, every `.cortex/` directory's `_index.md` active-prompt template, the archive module's index/register templates, the spec-tree skeleton templates, the CLAUDE.md managed block, the FIVE canonical Desktop scheduled-task BUNDLE definitions plus their SKILL.md renderer, and the compass/pulse skeleton leaf templates. New this pass (spec `core-cli.init-profile`, upgrading this entry from L2 to L3): `scopeTaskToProfile(task, profile)` — the one function in this file with actual branching logic, not just string assembly — scopes a `ScheduledTask` bundle to a process profile. `ScheduledTask` gained three optional fields (`specflowOnlyMembers`, `specflowOnlySkills`, `profiles`) that three of the five `SCHEDULED_TASKS` bundles (`daily`, `weekly-quality`, `test-runner`) now set, naming which of their member loops are spec-tree-dependent (Bucket-3) versus profile-agnostic (Bucket-1). No other shape change; the rest of the file remains pure string assembly — the shapes themselves are owned by cortex-schema.md.
...
- This pass's only other change (per the file's own doc comment, now superseded by the profile-scoping addition above) was prose inside three `SCHEDULED_TASKS` bundle bodies — no shape change from that edit alone.
...
- src/cli/scaffold.ts: `claudeMdBlock`, `scheduledTaskSkillMd`, `ScheduledTask` (type), `SCHEDULED_TASKS`, `scopeTaskToProfile` (NEW) — the Rules 10/13/17 mechanism this module's templates feed
- src/cli/sync.ts: `SCHEMA_VERSION`, `CORTEX_INDEXES`, `ARCHIVE_INDEX_TEMPLATE`, `INSIGHT_INDEX_TEMPLATE`, `SCHEDULED_TASKS`, `scopeTaskToProfile` (NEW), `scheduledTaskSkillMd` — the index-refresh and scheduled-task-payload-refresh comparisons
