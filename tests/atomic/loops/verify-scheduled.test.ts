...
describe('Path-convention matching (schema §3, capability level optional §2.1)', () => {
  ...
  it('a capability-level leaf is covered by EITHER shape on disk', async () => {
    ...
    writeAt(rootA, 'specs-business/d/o.business.md', bizSpec('d.o'));
    writeAt(rootA, 'specs/d/c/l.spec.md', devSpec('d.c.l', '../../../specs-business/d/o.business.md'));
    ...
    writeAt(rootB, 'specs-business/d/o.business.md', bizSpec('d.o'));
    writeAt(rootB, 'specs/d/c/l.spec.md', devSpec('d.c.l', '../../../specs-business/d/o.business.md'));
...
describe('Covered spec is silent, uncovered is named', () => {
  it('names c.d\'s two missing paths and not a.b', async () => {
    ...
    writeAt(root, 'specs-business/a/out.business.md', bizSpec('a.out'));
    writeAt(root, 'specs/a/b.spec.md', devSpec('a.b', '../../specs-business/a/out.business.md'));
    writeAt(root, 'specs/c/d.spec.md', devSpec('c.d', '../../specs-business/a/out.business.md'));
    ...
    expect(body).toContain('`c.d` (`specs/c/d.spec.md`): missing `tests/atomic/c/d.test.ts`, `tests/spec/c/d.test.ts`');
...
describe('Journey and scenario coverage checked per business spec', () => {
  it('a business spec with no journey test and absent from every covers: gets both gaps reported', async () => {
    ...
    writeAt(root, 'specs-business/x/y.business.md', bizSpec('x.y'));
    await runVerifyScheduled(root);
    const body = report(root);
    expect(body).toContain('`x.y` (`specs-business/x/y.business.md`)');
...
describe('Declared deferrals separated', () => {
  it('a dev spec whose Notes carry the journey-deferral convention puts its journey gap under "Deferred by decision"', async () => {
    ...
    writeAt(root, 'specs-business/h/safe.business.md', bizSpec('h.safe'));
    writeAt(
      root,
      'specs/h/hook.spec.md',
  ...
  it('a business spec\'s own Notes can declare the deferral too', async () => {
    ...
    writeAt(root, 'specs-business/h/safe.business.md', bizSpec('h.safe', 'Journey-layer tests deferred to v1.1.'));
    writeAt(root, 'tests/scenario/specs/s.md', scenarioSpec('s', ['h.safe']));
    await runVerifyScheduled(root);
    const body = report(root);
...
describe('Only the report is written', () => {
  it('a run touches only pulse/verification-report.md', async () => {
    ...
    writeAt(root, 'specs-business/x/y.business.md', bizSpec('x.y'));
    writeAt(root, 'specs/c/d.spec.md', devSpec('c.d', '../../specs-business/x/y.business.md'));
