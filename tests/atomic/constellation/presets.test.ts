 ...
 * a cerebrum core file, an atlas artefact, dev+business specs in domains
  ...
  return {
    ...
    groups: [
      ...
      { id: 'atlas', label: 'Atlas', children: [{ id: 'atlas:decisions', label: 'decisions' }] },
      {
        ...
        label: 'Cerebrum',
        children: [
          { id: 'cerebrum:bugs', label: 'bugs' },
          { id: 'cerebrum:core-files', label: 'core files' },
          { id: 'cerebrum:rules', label: 'rules' },
        ],
      },
      {
        id: 'specs',
    ...
    nodes: [
      ...
      { id: 'bug:B-001', module: 'bug', label: 'Fixture bug', group: 'cerebrum:bugs', ref: 'B-001' },
      { id: 'business:schema.trust', module: 'spec-business', label: 'schema.trust', group: 'specs:schema', ref: 'schema.trust' },
      { id: 'cerebrum:preferences.md', module: 'cerebrum', label: 'preferences.md', group: 'cerebrum:core-files', ref: '.cortex/cerebrum/preferences.md' },
      { id: 'rule:R-001', module: 'rule', label: 'Pure core', group: 'cerebrum:rules', ref: 'R-001' },
    ...
    edges: [
      { from: 'atlas:D-001', to: 'rule:R-001', kind: 'cerebrum_rules' },
    ...
    counters: { anatomy: 2, cerebrum: 3, atlas: 1, specs: 3, edges: 6, droppedRefs: 4 },
  };
}
...
describe('applyPreset: default', () => {
  ...
  it('recomputes counters over the (full) sets', () => {
    ...
    expect(out.counters).toEqual({ anatomy: 2, cerebrum: 3, atlas: 1, specs: 3, edges: 6, droppedRefs: 4 });
  });
...
describe('applyPreset: anatomy-only', () => {
  ...
  it('recomputes counters over the filtered sets, passing droppedRefs through', () => {
    const out = unwrap(applyPreset(makeConstellation(), 'anatomy-only'));
    expect(out.counters).toEqual({ anatomy: 2, cerebrum: 0, atlas: 0, specs: 0, edges: 0, droppedRefs: 4 });
  });
});

describe('applyPreset: knowledge-only', () => {
  it('excludes anatomy and keeps rule/bug/cerebrum/atlas/spec-dev/spec-business', () => {
    ...
    expect(new Set(out.nodes.map((n) => n.module))).toEqual(
      new Set(['rule', 'bug', 'cerebrum', 'atlas', 'spec-dev', 'spec-business']),
    );
  });
  ...
  it('prunes the anatomy group entirely, keeps the other three', () => {
    const out = unwrap(applyPreset(makeConstellation(), 'knowledge-only'));
    expect(out.groups.map((g) => g.id)).toEqual(['atlas', 'cerebrum', 'specs']);
  });
});
...
describe('applyPreset: orphans', () => {
  it('keeps only nodes with zero connected edges in EITHER direction', () => {
    ...
    expect(out.nodes.map((n) => n.id)).toEqual(['anatomy:src/cli/lone.ts', 'cerebrum:preferences.md']);
  });
  ...
  it('prunes to the groups still holding an orphan', () => {
    const out = unwrap(applyPreset(makeConstellation(), 'orphans'));
    expect(out.groups.map((g) => g.id)).toEqual(['anatomy', 'cerebrum']);
    expect(out.groups.find((g) => g.id === 'cerebrum')?.children).toEqual([
      { id: 'cerebrum:core-files', label: 'core files' },
    ]);
  });
});
...
describe('applyPreset: domain', () => {
  it('keeps only spec-dev/spec-business nodes whose id domain segment matches', () => {
    ...
    expect(out.counters).toEqual({ anatomy: 0, cerebrum: 0, atlas: 0, specs: 2, edges: 1, droppedRefs: 4 });
  });
  ...
  it('a domain matching nothing is an honest empty lens: 200-shaped, empty sets, zeroed counters', () => {
    ...
    expect(out.edges).toEqual([]);
    expect(out.groups).toEqual([]);
    expect(out.counters).toEqual({ anatomy: 0, cerebrum: 0, atlas: 0, specs: 0, edges: 0, droppedRefs: 4 });
  });
