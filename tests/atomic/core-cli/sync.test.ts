...
describe('Rule 4: an _index.md in a directory sync has no known template for is left alone', () => {
  ...
  it('a template-plus-recall-block atlas index is current, not localised, and keeps its block (recall.index-blocks Rule 7)', async () => {
    const { root, home } = await bootstrap('r4-recall-block');
    const { CORTEX_INDEXES, SCHEMA_VERSION } = await import('../../../src/cli/templates.js');
    const { RECALL_BLOCK_START, RECALL_BLOCK_END } = await import('../../../src/recall/index-blocks.js');
    const blocked =
      CORTEX_INDEXES['atlas/decisions'] +
      '\n' +
      RECALL_BLOCK_START(SCHEMA_VERSION) +
      '\n**Recorded (generated by `cortex scan`; newest first):**\n- 2026-08-05 decision.2026-08-05-b — Pull-only stance reversed · bears on: R-003\n' +
      RECALL_BLOCK_END +
      '\n';
    const indexPath = path.join(root, '.cortex', 'atlas', 'decisions', '_index.md');
    fs.writeFileSync(indexPath, blocked);
    const result = await sync(root, { home, ...DARWIN_SYNC });
    expect(result.exitCode).toBe(0);
    ...
    expect(fs.readFileSync(indexPath, 'utf-8')).toBe(blocked);
    cleanTmp(root); cleanTmp(home);
  }, TEST_TIMEOUT);
...
describe('Rules 6 & 7: sync repairs missing hooks/git-hook exactly like init would', () => {
  ...
  it('a project initialised before the Grep|Bash row gains `cortex hook search-annotate` on sync, once, with the other PreToolUse entries untouched; a second sync is idempotent', async () => {
    const { root, home } = await bootstrap('r67-search-annotate');
    const settingsPath = path.join(root, '.claude', 'settings.json');
    const settings = JSON.parse(fs.readFileSync(settingsPath, 'utf-8'));
    const searchRow = { matcher: 'Grep|Bash', hooks: [{ type: 'command', command: 'cortex hook search-annotate' }] };
    const others = (settings.hooks.PreToolUse as unknown[]).filter((e) => !JSON.stringify(e).includes('search-annotate'));
    expect(others.length).toBe(settings.hooks.PreToolUse.length - 1);
    settings.hooks.PreToolUse = others;
    fs.writeFileSync(settingsPath, JSON.stringify(settings, null, 2) + '\n');
    expect(fs.readFileSync(settingsPath, 'utf-8')).not.toContain('cortex hook search-annotate');

    const first = await sync(root, { home, ...DARWIN_SYNC });
    expect(first.exitCode).toBe(0);
    const after = JSON.parse(fs.readFileSync(settingsPath, 'utf-8'));
    expect(after.hooks.PreToolUse).toEqual([...others, searchRow]);
    for (const ev of ['SessionStart', 'PostToolUse', 'SessionEnd', 'Stop']) {
      expect(after.hooks[ev]).toEqual(settings.hooks[ev]);
    }
