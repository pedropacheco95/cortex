...
description: >
  ...
  the root cause layer, classify the bug type, document it in bugs.md, and produce a concrete
...
**This skill diagnoses. It does not fix.** The output is a classified bug entry in `bugs.md`
with a concrete change plan. The change-router executes the plan.
...
Every diagnosed bug is appended to `bugs.md` at the project root:
...
Bug IDs are sequential: BUG-001, BUG-002, etc. Read `bugs.md` to find the last number
and increment. If `bugs.md` doesn't exist, create it and start at BUG-001.

### Severity classification

| Severity | Criteria |
|----------|----------|
| Major | Data integrity at risk, security issue, core flow completely broken, users cannot accomplish primary outcome |
| Normal | Feature doesn't work as specified, but workaround exists or impact is limited |
| Minor | Cosmetic, edge case with minimal user impact, inconsistency that doesn't affect outcomes |
...
4. **Document each failure** as a separate bug entry in `bugs.md`.
...
- **Document everything in bugs.md.** Even if the bug is trivial. The bug log is the
  project's memory of what went wrong and why.
- **Classify severity honestly.** Not everything is major. A cosmetic issue is minor even
  if the user is frustrated about it.
