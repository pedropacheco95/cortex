...
- **Two parallel spec trees:**
  - `specs/` — developer specs (schemas, APIs, dependency chains, Given/When/Then acceptance criteria). Every implemented behaviour has a leaf spec here.
  - `specs-business/` — business specs (user-visible outcomes, journeys, business rules, success metrics). Stakeholder-readable. Each links to the dev specs that realise it via `implemented_by:`; each dev spec links back via `implements:`.
