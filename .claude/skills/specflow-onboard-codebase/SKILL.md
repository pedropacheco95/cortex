...
- Delta analysis: bugs.md, implicit-behaviors.md, dead-features.md, link-map.md
...
- **bugs.md** — Sequential IDs (BUG-001, BUG-002...). Each entry: severity, spec ID,
  investigation reference, what code does, what it should do, defense attempted, why it
  failed. Severities: major (data/security/core-flow), normal (broken with workaround),
...
| Fail — code wrong | Code doesn't match spec | Add to bugs.md |

After calibration: every failing test = bugs.md entry, every passing test = confirmed
alignment.
...
2. **Phase 1: Major bug triage** — all major-severity bugs from bugs.md
3. **Phase 2: Robustness** — normal-severity bugs, input validation gaps
...
Bugs: [N] ([N] major, [N] normal, [N] minor)
